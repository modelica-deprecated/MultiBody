package Sensors "Sensors to measure variables"
  model AbsoluteSensor
    ...
    extends Interfaces.PartialAbsoluteSensor(outPort(n=3*((if get_r_abs then 1
             else 0) + (if get_v_abs then 1 else 0) + (if get_a_abs then 1
             else 0) + (if get_angles then 1 else 0) + (if get_w_abs then 1
             else 0) + (if get_z_abs then 1 else 0))));
...
<IMG SRC=\"../Images/Sensors/CutForce.png\">
  ...
  equation
    ...
    elseif resolveInFrame_a then
      ...
      force = Frames.resolve1(frame_a.R, frame_a.f)*(if positiveSign then +1
         else -1);
    end if;
...
<IMG SRC=\"../Images/Sensors/CutTorque.png\">
  ...
  equation
    ...
    elseif resolveInFrame_a then
      ...
      torque = Frames.resolve1(frame_a.R, frame_a.t)*(if positiveSign then +1
         else -1);
    end if;
