...
If the underlying description of the orientation description
is changed, then only changes in this package are necessary.
</p>
<h4>Content</h4>
...
<pre>
   Orientation R, R1, R2, R_rel, R_inv;
   Real[3,3]   T, T_inv;
...
<table border=1 cellspacing=0 cellpadding=2>
  ...
  <tr><td><b>der_Orientation</b> der_R;</td>
  ...
  <tr><td>w2 = <b>angularVelocity2</b>(R, der_R);</td>
      <td>Return angular velocity resolved in frame 2 from
          orientation object R<br> and its derivative der_R.
     </td>
  </tr>
  ...
  <tr><td>[v1,w1] = <b>multipleResolve1</b>(R, [v2,w2]);</td>
      <td>Transform several vectors from frame 2 to frame 1.
      </td>
  </tr>
  <tr><td>[v2,w2] = <b>multipleResolve2</b>(R, [v1,w1]);</td>
  ...
  <tr><td>R = <b>planarRotation</b>(e, angle);</td>
  ...
  <tr><td>R = <b>axisRotation</b>(i, angle);</td>
      <td>Return orientation object R for rotation around axis i of frame 1.
      </td>
  </tr>
  <tr><td>R = <b>axesRotations</b>(sequence, angles);</td>
      <td>Return rotation object to rotate in sequence around 3 axes. Example:<br>
          R = axesRotations({1,2,3},{90,45,-90});
      </td>
  </tr>
  ...
  type Orientation
    "Orientation type defining rotation from a frame 1 into a frame 2"
...
<pre>
   <b>R</b> = [<b>e</b><sub>x</sub>, <b>e</b><sub>y</sub>, <b>e</b><sub>z</sub>];
       e.g., <b>R</b> = [1,0,0; 0,1,0; 0,0,1]
</pre>
<p>
...
<pre>
    <b>v</b><sub>2</sub> = <b>R</b> * <b>v</b><sub>1</sub>
</pre>
</p>
The <b>inverse</b> orientation
<b>R_inv</b> = <b>R</b><sup>T</sup> describes the rotation
...
in package Frames. As a consequence, other implementations of
...
"));
    ...
    algorithm
      residue := {cross(R1[1, :], R1[2, :])*R2[2, :],-cross(R1[1, :], R1[2, :])
        *R2[1, :],R1[2, :]*R2[1, :]};
  ...
  algorithm
    residue := {R[:, 1]*R[:, 1] - 1,R[:, 2]*R[:, 2] - 1,R[:, 3]*R[:, 3] - 1,R[:,
       1]*R[:, 2],R[:, 1]*R[:, 3],R[:, 2]*R[:, 3]};
  ...
  function angularVelocity2
    ...
    extends Modelica.Icons.Function;
    input Orientation R "Orientation object to rotate frame 1 into frame 2";
    input der_Orientation der_R "Derivative of R";
    output Modelica.SIunits.AngularVelocity w[3]
      "Angular velocity of frame 2 with respect to frame 1 resolved in frame 2";
  algorithm
    /* The absolute angular velocity of frame_a, resolved in frame_a, is defined as:
        w_a = vec(transpose(T)*der(T));
     where
                   |   0 -w3  w2 |
     ...
     i.e.
         W   = transpose(T)*der(T)
         w_a = {W(3,2), -W(3,1), W(2,1)}
     Therefore, only 3 values of W need to be computed:
             | e_x' |
         W = | e_y' | * | der(e_x), der(e_y), der(e_z) |
             | e_z' |
               |  W(3,2) |   |  e_z'*der(e_y) |
         w_a = | -W(3,1) | = | -e_z'*der(e_x) |
               |  W(2,1) |   |  e_y'*der(e_x) |
  */
    w := {R[3, :]*der_R[2, :],-R[3, :]*der_R[1, :],R[2, :]*der_R[1, :]};
  ...
  algorithm
    R := [e]*transpose([e]) + (identity(3) - [e]*transpose([e]))*Math.cos(angle)
...
and/or a division by zero will occur.
  ...
  function axisRotation
    ...
    import Modelica.Math.*;
    extends Modelica.Icons.Function;
    input Integer i(min=1, max=3) "Rotate around axis i of frame 1";
    input Modelica.SIunits.Angle angle
      "Rotation angle to rotate frame 1 into frame 2 along axis i of frame 1";
    output Orientation R "Orientation object to rotate frame 1 into frame 2";
  algorithm
    R := if i == 1 then [1, 0, 0; 0, cos(angle), sin(angle); 0, -sin(angle),
  ...
  algorithm
    R := absoluteRotation(absoluteRotation(axisRotation(sequence[1], angles[1]),
       axisRotation(sequence[2], angles[2])), axisRotation(sequence[3], angles[
      3]));
  end axesRotations;
  ...
  protected
    ...
    Orientation R_1a "Orientation object to rotate frame 1 into frame 1a";
  algorithm
  ...
  */
    ...
    R_1a := planarRotation(e1_1, angles[1]);
    angles[2] := Frames.planarRotationAngle(e2_1a, Frames.resolve2(R_1a, e3_1),
       e3_2);
    angles[3] := Frames.planarRotationAngle(e3_2, e2_1a, Frames.resolve2(R,
      Frames.resolve1(R_1a, e2_1a)));
    annotation (Documentation(info="<HTML>
<p>
A call to this function of the form
</p>
    ...
    /* Planar rotation:
       ...
       Trel = [1,      phi3,   -phi2;
               -phi3,     1,    phi1;
                phi2, -phi1,       1 ];
  */
    phi := if withResidues then {R[2, 3],-R[1, 3],R[1, 2],R[1, 1] - 1,R[2, 2]
       - 1,R[1, 1]*R[2, 2] - R[2, 1]*R[1, 2] - 1} else {R[2, 3],-R[1, 3],R[1, 2]};
  end smallRotation;
...
arbitrarily such that n_x and e_z are orthogonal to each other.
  ...
  T := (2*Q[4]*Q[4] - 1)*identity(3) + 2*([Q[1:3]]*transpose([Q[1:3]]) - Q[4]*
    skew(Q[1:3]));
*/
    R := [2*(Q[1]*Q[1] + Q[4]*Q[4]) - 1, 2*(Q[1]*Q[2] + Q[3]*Q[4]), 2*(Q[1]*Q[3]
       - Q[2]*Q[4]); 2*(Q[2]*Q[1] - Q[3]*Q[4]), 2*(Q[2]*Q[2] + Q[4]*Q[4]) - 1,
      2*(Q[2]*Q[3] + Q[1]*Q[4]); 2*(Q[3]*Q[1] + Q[2]*Q[4]), 2*(Q[3]*Q[2] - Q[1]
      *Q[4]), 2*(Q[3]*Q[3] + Q[4]*Q[4]) - 1];
  end from_Q;
  ...
  function to_Q
    ...
    output Quaternions.Orientation Q
      "Quaternions orientation object to rotate frame 1 into frame 2";
  algorithm
    Q := Quaternions.from_T(R, Q_guess);
  end to_Q;
  ...
  algorithm
    vec := {R[1, 1],R[2, 1],R[3, 1],R[1, 2],R[2, 2],R[3, 2],R[1, 3],R[2, 3],R[3,
       3]};
  end to_vector;
...
"));
      ...
      encapsulated function equalityConstraint
        ...
        import Modelica;
        import MultiBody.Frames.Quaternions;
        extends Modelica.Icons.Function;
   ...
   There is an ambiguity for Euler parameters, since Q and -Q
